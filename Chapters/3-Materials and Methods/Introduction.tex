...
Pyrolysis
\item Biochar and soil characterization
\item 	Total C, H, N
\item 	SSA, pore volume
\item 	iron speciation
\item Batch leaching test
\item 	biochar-water (L/S 500)
\item 		single-compound
\item 		cocktail
\item 	soil-biochar-water (L/S 10, 2\% biochar dose of d.w.)
\item 		PFOA single-compound
\item 		cocktail
\item 	blanks
\item 		soil and cocktail
\item 		soil extraction, PFAS contamination
...
Elemental composition (C, H and N) was quantified with a Leco CHN-1000 from Leco Corporations (Sollentuna, Sweden), according to DIN 51732. 

Iron speciation of DSL and ULS biochars was analyzed at the ESRF facility (Grenoble, France) by a project colleague at the Norwegian Geotechnical Institute. The biochars were crushed using a mortar to \textless 250 \textmu m fine powder (sieved and re-crushed several times) until no visible particles were present, mixed ten times with boron nitride (BN) to a homogeneous powder and pelletized. The pellet was then analyzed with an XRF beam. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Batch tests}
Sorption isotherm experiments were carried out to characterize the sorption capacity of CWC, ULS, and DSL to the six selected PFCAs using one-step aqueous batch shaking tests with biochar and water at a liquid to solid mass ratio (L/S) of 500, and soil amended with 2\% biochar at L/S of 10 according to CEN EN 12457


\subsection{Design of sorption isotherms}
Determination of spike concentrations for each TA was taken as a trade-off between convenience of preparing standard solution at a certain concentration and biochar dose together with predicted biochar-water partition coefficients, $K_{BC}$ and the instrumental LOQs in LC-MS/MS. $K_{BC}$ values used for the calculations were based on a series of sorption experiments conducted on five PFCAs to activated carbon by \cite{Xiao2017}. The $K_{BC}$'s used in the calculations and are listed in \cref{tab:Kbc}. The right balance between these parameters is important to ensure detectable $C_w$ across a wide enough concentration range to see a sorption trend on the isotherm. A 10$^4$ concentration range was selected. The relationship between $K_{BC}~(L g^{-1})$, the sorbed concentration, $C_s~(\mu g~g^{-1})$, and the freely dissolved aqueous concentration, $C_w~ (\mu g~L^{-1})$ is expressed as:
...
The char-water suspensions were assured to contain \textless 10\% MeOH. The sorbent-sorbate mixtures were shaken end-over-end (9 rpm) and/or agitated on a shaking table (160 rpm) in room temperature (20\textdegree C) for at least 14 days to reach equilibrium  \citep{higgins} Why is it important to reach equilibrium in this study? Not kinetics we are looking at. 
...
One-step aqueous batch tests were carried out to characterize sorption by CWC, ULS, and DSL to the selected PFCAs in the presence of soil. The batch tests were prepared as 6-point isotherms within a 10\textsuperscript{4} concentration range with a L/S ratio of 10, in accordance with CEN EN 12457 with modifications described in \citep{Hale2017fire,Kupryianchyk2016a}. L/S 10  is common in leaching tests for waste materials and soil according to the waste and shaked in an end-over-end shaker for \textgreater 14 days. Prepared in PP tubes. 
