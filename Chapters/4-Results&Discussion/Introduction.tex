...
\cref{tab:SAPV} shows the total surface area (SA) and total pore volume (PV) for the three biochars used in the sorption experiments in this study. For small pores (0.4-1.5 nm, CO\textsubscript{2} sorptometry), SA for CWC was $\sim$six times higher than ULS and DSL (165 and 87  m\textsuperscript{2} g\textsuperscript{-1}, respectively), and pore volume also followed the same order. This means that CWC has a much higher fraction of potential sorption sites compared to ULS and DSL for the smallest pores. This can be visualized by plotting the pore size distribution of the biochars in terms of surface area and pore volume. In \cref{fig:PZD_small} it becomes evident that the largest fraction of CWC surface area is allocated in nanopores $<$ 0.6 nm, the sorption volume is relatively low, dominated by small micropores. 

However, the SA/PV ratio yields similar values (3 674, 3 502, and 3 206 m\textsuperscript{2} cm\textsuperscript{-3} for CWC, ULS, and DSL respectively) across biochar type. The SA/PV ratio indicates how much pore space there is for sorption, where a high ratio means that the pores have large surface areas but little pore volume. The SA/PV ratio becomes important for sorption of PFAS because these compound classes are quite rigid molecularly, meaning that the the perfluorinated chains are not flexible enough to fit into small pores. Additionally, molecular size increases with chain length, which makes it even more difficult for long-chain PFASs to diffuse into small pores. \cref{tab:molecsize} lists effective cross-sectional diameter ($D_{eff}$) and maximum diameter ($D_{max}$) of each TC, and \cref{fig:molecularSize} illustrates the definition of these parameters. PFCAs C5-C10 range from 0.45-0.72 nm $D_{eff}$ and 0.96-1.54 nm $D_{max}$. Maximum sorption is acheived when the molecular dimensions of the PFAS congeners match the pore size and shape \citep{Hale2016}. Large and congeners may then experience size exclusion due to steric hindrance  Size exclusion This means that pores between 0.4-1.5 nm have quite limited pore space for PFAS to diffuse into, and will likely experience size exclusion. Since CWC is similar in SA/PV ratio to the sludge chars and all ratios are relatively high, sorption of PFAS in small pores cannot explain the differences in sorption between biochar types. (Some words about previous literature on significance of SA and PV for sorption) \citep{Hale2016}:  Maximum sorption occurs when the molecular dimensions of the PFAS congeners fit inside the pores called size exclusion. Size exclusion is particularly prominent for biochars that are dominated with small micropores, 

Surface area (SA) for the largest pore fraction ($>$ 1.5 nm) increased in the order: DSL $<$ ULS $<$ CWC, (110, 128, and 323 m\textsuperscript{2} g\textsuperscript{-1} respectively), whereas pore volume (PV) was highest for ULS (0.126 cm\textsuperscript{3} g\textsuperscript{-1}) and as much as one order of magnitude lower for CWC (0.017 cm\textsuperscript{3} g\textsuperscript{-1}). This shift in trend affects the SA/PV ratio significantly giving a high ratio for CWC (1 633 $\mathrm{m^2 cm^{-3}}$) versus lower and similar values for ULS and DSL (631 and 634 $\mathrm{m^2 cm^{-3}}$, respectively). 


Taking the ratio between SSA and PV gives an indication of the average pore size for the three biochars, which increases in the order: ULS $<$ DSL $<$ CWC (631, 634, and 1 633 m\textsuperscript{2} cm\textsuperscript{-3}, respectively), where a small ratio indicates a higher fraction of larger pore spaces. A small SA/PV ratio shows to be the most significant relationship to sorption strength, and can possibly explain why ULS is the strongest sorbent among the three biochar samples tested in this study. There is only a small difference (3 m\textsuperscript{2} cm\textsuperscript{-3}) between the SA/PV ratio of ULS and DSL, so another parameter must explain why there is a difference in sorption strength between these chars.  To explain why ULS is a stronger sorbent than DSL, one can consider the difference in carbon content, where ULS consists of 29.6 \% C versus 13.5 \% for DSL. 
...
\subfloat[\label{subfig:SA_small}]{%
  \includegraphics[width=0.45\textwidth]{R/figs/SA_small.pdf}
}
\hfill
\subfloat[\label{subfig:PV_small}]{%
  \includegraphics[width=0.45\textwidth]{R/figs/PV_small.pdf}
}
...
sorbent & temperature & \multicolumn{3}{l}{(pores \textgreater 1.5 nm)} & \multicolumn{3}{l}{(pores 0.4-1.5 nm)} & & & & & & & \\ \cmidrule(l){3-5} \cmidrule(l){6-8} \cmidrule(l){9-12} \cmidrule(l){13-15} & (\textdegree C) & SA  & PV & SA/PV & SA & PV & SA/PV & C & O & H & N & O/C & H/C & N/C \\
...
CWC & 700 & 323 & 0.017 & 1 633 & 683 & 0.186 & 3 502 & 91.4 & 5.50 & 1.01 & 0.69 & 0.06 & 0.01 & 0.008       \\
ULS & 700 & 128 & 0.126 & 631 & 165 & 0.047 & 3 674 & 29.6 & 57.1 & 1.24 & 1.13 & 1.9  & 0.04 & 0.04        \\
DSL & 700 & 110 & 0.111 & 634 & 87  & 0.027 & 3 206 & 13.5 & 61.4 & 1.05 & 0.82 & 4.6  & 0.08 & 0.06       \\ \bottomrule
\end{tabular}}
\end{table}
...
\subsection{Degree of aromaticity/carbon content/main elements}
...
\subfloat[\label{subfig:Kd_C}]{%
  \includegraphics[width=0.32\textwidth]{R/figs/Kd_1ugL_C.pdf}
}
\hfill
\subfloat[\label{subfig:Kd_SAPV}]{%
  \includegraphics[width=0.32\textwidth]{R/figs/Kd_1ugL_SA_PV.pdf}
}
\hfill
\subfloat[\label{subfig:Kd_SAPVC}]{%
  \includegraphics[width=0.32\textwidth]{R/figs/Kd_1ugL_SA_PV_C.pdf}
}
\caption{(a) Plot of carbon content versus $log~K_d$ (b) Plot of surface area to pore volume ratio versus $log~K_d$. (c) plot of (SA/PV)/C ratio, shows that individually, a and b does not explain sorption, but together they do. Error bars are the propagated standard error, $log~K_d$ normalized to 1 $\mu g L^{-1}$.}
\label{fig:Kd_SAPV_C}
\end{figure}
