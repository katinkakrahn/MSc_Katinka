...
\cref{tab:SAPV} shows the total surface area (SA) and total pore volume (PV) for the three biochars used in the sorption experiments in this study. For pores 0.4-1.5 nm, SA for CWC was $\sim$six times higher than ULS and DSL (165 and 87  m\textsuperscript{2} g\textsuperscript{-1}, respectively), and pore volume also followed the same order. This means that CWC has a much higher fraction of sorption sites compared to ULS and DSL for the smallest pores. However, the SA/PV ratio yields similar values (3 674, 3 502, and 3 206 m\textsuperscript{2} cm\textsuperscript{-3} for CWC, ULS, and DSL respectively) across biochar type. The SA/PV ratio indicates how much pore space there is for sorption, where a high ratio means that the pores have large surface areas but little pore volume. The SA/PV ratio becomes important for sorption of PFAS because these compound classes are quite rigid molecularly, meaning that the the perfluorinated chains are not flexible enough to fit into small pores. Additionally, molecular size increases with chain length, which makes it utterly difficult for long-chain PFASs to diffuse into small pores. \cref{tab:molecsize} lists effective cross-sectional diameter ($D_{eff}$) and maximum diameter ($D_{max}$) of each TC, and \cref{fig:molecularSize} illustrates the definition of these parameters. This means that pores between 0.4-1.5 nm have quite limited pore space for PFAS to diffuse into. Since CWC is similar in SA/PV ratio to the sludge chars and all ratios are relatively high, sorption of PFAS in small pores cannot explain the differences in sorption between biochar types. (Some words about previous literature on significance of SA and PV for sorption)
...
\subfloat[\label{subfig:SAPV_large}]{%
  \includegraphics[height=6.5cm]{R/figs/SAPV_large.pdf}
}
\hfill
