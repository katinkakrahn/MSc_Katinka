...
In \cref{fig:sorption_isotherms}, the sorption isotherms are presented for PFPeA, PFHxA, PFHpA, PFOA, PFNA and PFDA to CWC, ULS and DSL. The points generated from the batch tests were interpreted with the Freundlich equation (\cref{eq:FreundlichLinear}). For all compounds, the sorption isotherm for ULS is visibly higher than DSL, followed by CWC. The Freundlich sorption coefficients ($log~K_F$), linearity coefficients ($n$) and correlation coefficients ($r^2$) are presented in \cref{tab:summary_stats_single}. Somewhat unexpectedly, sorption is strongest for the two sludge chars. Possible reasons for the strong sorption by the sludge biochars will be discussed in this chapter. The Freundlich coefficients for ULS increased in the order PFPeA $<$ PFHxA $<$ PFOA $<$ PFHpA $<$ PFNA $<$ PFDA, which ranges from $log~K_F$ = 4.10-6.00. All regressions were significant (p$<$0.01) or highly significant (p$<$0.001). The Freundlich coefficients for DSL increased in the order PFHxA $<$ PFPeA $<$ PFHpA $<$ PFOA $<$ PFNA $<$ PFDA, and ranges from $log~K_F$ = 3.16-5.61. All regressions were highly significant (p$<$0.001) except for PFPeA. The Freundlich coefficients for CWC increased in the order PFHxA $<$ PFPeA $<$ PFHpA $<$ PFNA $<$ PFOA $<$ PFDA, which ranges from $log~K_F$ = 3.01-5.22. All regressions were significant (p$<$0.01) or highly significant (p$<$0.001) except for PFPeA. 

Due to the novel experiments conducted in this thesis, literature partitioning coefficients for PFCAs of other sewage sludge biochars are non-existent (is that correct?). However, a comparison can be made between the $log~K_F$s found in this thesis and for commercially-produced activated carbons from other studies. The sorption coefficients are equivalent or higher compared to literature values for $log~K_F$ of PFOA to commercially-produced activated carbons: 5.60 \citep{Kupryianchyk2016a}, 4.45 \citep{hansen2010sorption}, and 4.74-5.42 \citep{silvani2019can}. This is promising for the potential to replace non-renewable activated carbons with waste based biochars for environmental remediation in the field and wastewater treatment. 
...
\cref{fig:sorption_isotherms_all} show sorption isotherms for the single-compound batch tests for CWC, ULS and DSL separately.
