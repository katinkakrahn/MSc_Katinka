...
In \cref{fig:sorption_isotherms}, the sorption isotherms are presented for PFPeA, PFHxA, PFHpA, PFOA, PFNA and PFDA to CWC, ULS and DSL. The isotherms were interpreted with the Freundlich equation (\cref{eq:FreundlichLinear}). For all compounds, the sorption isotherm for ULS is visibly higher than DSL followed by CWC. The Freundlich sorption coefficients ($log~K_F$), linearity coefficients ($n$) and correlation coefficients ($r^2$) are presented in \cref{tab:summary_stats_single}. Surprisingly, sorption increases in the order, CWC \textgreater DSL \textgreater ULS, attributing the highest sorption to the sewage sludge biochars. Possible reasons for the unexpected strong sorption by the sludge biochars will be discussed throughout this chapter. 
...
\begin{figure}[t]\ContinuedFloat
        ...
        \end{subfigure}
        \caption{Single-compound Freundlich sorption isotherms. Lines are obtained by linear regression.}
