...
Another parameter is needed to explain why ULS is a better sorbent than DSL in this study since there is only a minor difference in SA/PV ratio between these feedstocks (\cref{subfig:SAPV_large}). Apart from SA and PV, carbon content has shown to be a good predictor of sorption affinity to PFAS in previous literature \citep{Hale2016}. ULS consists of 29.6 \% C versus 13.5 \% for DSL, so by normalizing the SA/PV ratio for C-content a clear distinction in pore structure emerges (\cref{subfig:SAPV_C_large}). ULS has a lower (SA/PV)/C ratio, which means that the pore walls of ULS consists of a higher percentage C making the pore walls more hydrophobic and enhances sorption affinity of PFAS. \cref{fig:Kd_SAPV_C} also shows that individually, C and SA/PV does not explain sorption, but together they do.  
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{R/figs/Kd.pdf}
