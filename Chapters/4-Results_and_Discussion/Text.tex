...
\input{Chapters/4-Results_and_Discussion/Isotherms} 
...
where $K_{d,BC~single}$ is the mixed, soil mixed, or soil single batch test sample partition coefficient. AFs for PFPeA, PFHxA and PFHpA not presented due to lack of consistent results. Lowest AFs were measured for BC-soil-PFOA, where AFs were 3, 4, and 10 for CWC, DSL, and ULS respectively. All calculated AFs are in \cref{tab:attenuation}. It appears that ULS, which has the highest sorption, also has the highest attenuation. It was difficult to evaluate any trends in AFs between biochar samples or PFCA chain lengths, so further investigation is needed by conducting attenuation experiments with a larger biochar sample size. 
