...
\cref{fig:PFOA_attenuation} shows the effect of attenuation on the PFOA isotherms for BC soil single and BC soil mixed. The isotherms were spiked with the same $C_i$, only that six of the ten points were selected for the attenuation isotherms (\cref{sec:S-BC}. For all three biochars, BC single have highest sorption, followed by BC soil single, and BC soil mixed. Freundlich partition coefficients and slopes are in \cref{appSec:Sorption}\cref{apptab:summary_stats_all}. This trend gives a good representation of the hierarchy of factors that influence attenuation, consistent with the results discussed earlier in this section. The isotherms have similar $C_s$ concentrations but varying $C_w$, depending on sample type. Due to pore blocking and competitive sorption, more sorbate in solution is needed to push the same amount of molecules onto the solid phase in presence of soil. This effect is further enhanced by the presence of both soil and a cocktail---indicated by the green line being the lowest isotherm---because the PFCAs with longer chain lengths (PFNA and PFDA) are expected to be stronger in the competition for sorption sites than PFOA \citep{Sormo2021}. Consistent with \cref{fig:C10_AF}, the cocktail attenuation effect is more significant than attenuation by soil. 
...
\begin{figure}
    \centering
    \includegraphics{R/figs/PFOA_linear.pdf}
    \caption{Attenuation isotherms for PFOA plotted without log axes. Curves are fitted by a polylogarithmic curve (poly(log(x), 2)).}
