...
CWC_isotherm_all <- ggplot(data = CWC_single_all) +
  ...
  labs(x = expression(log~C[w]), y = expression(log~C[s]), col = "Compound", title = "CWC 10-point isotherm") + 
  theme_bw() +
...
ULS_isotherm_all <- ggplot(data = ULS_single_all) +
  geom_point(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound))) + 
  geom_smooth(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound)), formula = y ~ x, method=lm, se=FALSE, fullrange = TRUE) + 
  labs(x = expression(log~C[w]), y = expression(log~C[s]), col = "Compound", title = "ULS 10-point isotherm") + 
  theme_bw() +
  theme(legend.position = c(0.9, 0.15))
ULS_isotherm_all
...
ULS_isotherm_C1omit <- ggplot(data = ULS_single_C1omit) +
  geom_point(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound))) + 
  geom_smooth(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound)), formula = y ~ x, method=lm, se=FALSE, fullrange = TRUE)+ 
  labs(x = expression(log~C[w]), y = expression(log~C[s]), col = "Compound", title = "ULS isotherm C1 omitted") + 
  theme_bw() +
  theme(legend.position = c(0.9, 0.15))
ULS_isotherm_C1omit
...
BRL_isotherm_all <- ggplot(data = BRL_single_all) +
  geom_point(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound))) + 
  geom_smooth(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound)), formula = y ~ x, method=lm, se=FALSE, fullrange = TRUE) + 
  labs(x = expression(log~C[w]), y = expression(log~C[s]), col = "Compound", title = "BRL 10-point isotherm") + 
  theme_bw() +
  theme(legend.position = c(0.9, 0.15))
BRL_isotherm_all
...
BRL_isotherm_C1omit <- ggplot(data = BRL_single_C1omit) +
  geom_point(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound))) + 
  geom_smooth(mapping = aes(x = log_Cw, y = log_Cs, color = factor(Compound)), formula = y ~ x, method=lm, se=FALSE, fullrange = TRUE) +
  labs(x = expression(log~C[w]), y = expression(log~C[s]), col = "Compound", title = "BRL isotherm C1 omitted") + 
  theme_bw() +
  theme(legend.position = c(0.9, 0.15))
BRL_isotherm_C1omit
...
for(i in 1:nr_compounds){
  ...
  summary_stats_BRL_single_C1omit[compound == compounds[i], K_F_std_error := summary(fit)$coefficients[1,2]]
  summary_stats_BRL_single_C1omit[compound == compounds[i], K_F_t_value := summary(fit)$coefficients[1,3]]
  summary_stats_BRL_single_C1omit[compound == compounds[i], K_F_p_value_t := summary(fit)$coefficients[1,4]]
  summary_stats_BRL_single_C1omit[compound == compounds[i], n := fit$coefficients[2]]
  summary_stats_BRL_single_C1omit[compound == compounds[i], n_std_error := summary(fit)$coefficients[2,2]]
  summary_stats_BRL_single_C1omit[compound == compounds[i], n_t_value := summary(fit)$coefficients[2,3]]
  summary_stats_BRL_single_C1omit[compound == compounds[i], n_F_p_value_t := summary(fit)$coefficients[2,4]]
  ...
  summary_stats_BRL_single_C1omit[compound == compounds[i], p_value := pf(summary(fit)$fstatistic[1],summary(fit)$fstatistic[2],
                                                                       summary(fit)$fstatistic[3],lower.tail=F)]
}
summary_stats_BRL_single_C1omit
