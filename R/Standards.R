...
Standards <- read_excel("C:/Users/KMK/OneDrive - NGI/VOW/Data/180222_Standards_rawdata.xlsx")
...
Standard_conc <- ggplot(data = Standards_single_summary, aes(x = Compound, y = mean_conc)) +
  geom_point() + 
  geom_errorbar(aes(ymin=mean_conc-sd_conc, ymax=mean_conc+sd_conc), width=.1, position=position_dodge(.9)) +
